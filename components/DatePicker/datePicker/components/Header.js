...
const Header = ({ changeMonth }) => {
	const { options, disableDateChange, state, utils, minimumDate, maximumDate, isGregorian, mode } = useCalendar();
	...
	] = utils.useMonthAnimation(mainState.activeDate, options.headerAnimationDistance, () => setDisableChange(false));
	const prevDisable = disableDateChange || (minimumDate && utils.checkArrowMonthDisabled(mainState.activeDate, true));
	const nextDisable =
	...
	const onChangeMonth = type => {
		...
		const newDate = utils.getDate(mainState.activeDate).add(modificationNumber, isGregorian ? 'month' : 'jMonth');
		setMainState({
			type: 'set',
